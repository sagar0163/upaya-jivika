...
- ✅ Credentials storage — `src/vault.py` (CredentialsVault): API keys read from Render env vars; platform passwords persisted to a Supabase `credentials` table (auto-bootstrapped), with in-memory overrides + env fallback. Values are encrypted at rest (Fernet, key derived from `VAULT_ENCRYPTION_KEY`). **Fail-closed (issue #72):** with `SUPABASE_URL` set, the app refuses to boot and `vault.set()` refuses to write unless `VAULT_ENCRYPTION_KEY` is configured — a Supabase read-access leak must never expose plaintext `. Legacy plaintext rows written before the key existed stay readable for migration via `scripts/migrate_plaintext_vault.py` (encrypts every non-`enc:v1:` row under a supplied key). Local/CI plaintext is only available behind the explicit `ALLOW_PLAINTEXT_VAULT=1` opt-in.
...
| ✅ | API authentication | **SOLVED** — `src/api_auth.py` (`require_api_token`): every state-mutating endpoint (`/api/withdraw`, `/api/spend/{id}/reject`, `/api/debt/tick`, `/api/research/trigger`, `/api/email/scan`, `POST /api/survival-mode`) requires `Authorization: Bearer <API_AUTH_TOKEN>`; fails closed (503) if the token isn't configured, same pattern as the Payoneer webhook. Issue #74 closed the last public read surface: `/status`, `/api/spend/pending`, `/api/email/status`, `GET /api/survival-mode` and the `/ws` WebSocket (cookie / Bearer / `?token=` handshake, rejected with close codes 4401/4403) are now gated too, and `/health` is a bare public liveness probe (`{"status":"ok"}`) leaking no debt/life/earnings/research-trigger data. The dashboard shell itself stays public — it carries no state, and its JS prompts for the token before any data loads. The token is exchanged once for an httpOnly `uj_session` cookie (`POST /api/session`) that page script can never read — it never appears in the page source or JS-readable storage |
