# Issue #73 Plan
- [ ] Split `API_AUTH_TOKEN` into `PAYONEER_TX_TOKEN` (for manual confirmation) and `WITHDRAWAL_TOKEN` (for withdrawals).
...
- [ ] Add freshness check to `verify_signature` in `src/payoneer_webhook.py` (fail replayed webhooks > 5 minutes old).
